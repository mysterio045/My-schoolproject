...
// page through it locally; the backend `search` param matches name/phone/email.
...
function lastOrderLabel(customer: CustomerRecord): string {
  return customer.last_order_at
    ? getTimeAgo(customer.last_order_at)
    : "No orders";
}
...
export default function CustomersPage() {
  ...
  const [serverCustomers, setServerCustomers] =
    useState<PageResult<CustomerRecord> | null>(null);
  const [filteredCustomers, setFilteredCustomers] = useState<CustomerRecord[]>(
    []
  );
  const [statsResult, setStatsResult] = useState<CustomerStats | null>(null);
  ...
  const load = useCallback(async () => {
    ...
    try {
      const [statsRes, listRes] = await Promise.all([
        getCustomers({ page: 1, page_size: MAX_FILTER_ROWS }),
        isSearching
          ? getCustomers({
              page: 1,
          ...
          : getCustomers({ page: currentPage, page_size: ITEMS_PER_PAGE }),
      ]);
      const active = statsRes.items.filter((c) => c.status === "active").length;
      const inactive = statsRes.items.filter(
        (c) => c.status === "inactive"
      ).length;
      const totalRevenue = statsRes.items.reduce(
      ...
      if (isSearching) {
        setFilteredCustomers(listRes.items);
        setServerCustomers(null);
      } else {
        setServerCustomers(listRes);
  ...
  const rows = useMemo(() => {
    if (isSearching) {
      ...
      return filteredCustomers.slice(from, from + ITEMS_PER_PAGE);
    }
    return serverCustomers?.items ?? [];
  }, [isSearching, filteredCustomers, serverCustomers, currentPage]);

  const totalCount = isSearching
    ? filteredCustomers.length
    : (serverCustomers?.total ?? 0);
  const totalPages = isSearching
    ? Math.ceil(filteredCustomers.length / ITEMS_PER_PAGE)
    : (serverCustomers?.pages ?? 0);
  ...
  useEffect(() => {
    ...
    // eslint-disable-next-line react-hooks/set-state-in-effect
    void loadDetail(selectedCustomer.id);
  }, [selectedCustomer, loadDetail]);
            ...
            <div className="rounded-xl border border-[var(--border)] bg-[var(--card)] p-4 transition-colors hover:shadow-sm">
              <div className="flex items-start justify-between">
                <div>
                  ...
                  <p className="mt-1 text-2xl font-bold tracking-tight text-green-600 dark:text-green-400">{statsResult?.active ?? "—"}</p>
                </div>
            ...
            <div className="rounded-xl border border-[var(--border)] bg-[var(--card)] p-4 transition-colors hover:shadow-sm">
              <div className="flex items-start justify-between">
                <div>
                  ...
                  <p className="mt-1 text-2xl font-bold tracking-tight text-gray-500 dark:text-gray-400">{statsResult?.inactive ?? "—"}</p>
                </div>
            ...
            <div className="rounded-xl border border-[var(--border)] bg-[var(--card)] p-4 transition-colors hover:shadow-sm">
              <div className="flex items-start justify-between">
                <div>
                  ...
                  <p className="mt-1 text-2xl font-bold tracking-tight text-[var(--foreground)]">{statsResult ? formatNaira(statsResult.totalRevenue) : "—"}</p>
                </div>
                ...
                >
                  ...
                  <div className="grid grid-cols-3 gap-3 text-[12px]">
                    <div>
                      ...
                      <p className="font-medium text-[var(--foreground)]">{customer.total_orders}</p>
                    </div>
                    <div>
                      <p className="text-[var(--muted-foreground)]">Total Spent</p>
                      <p className="font-medium text-[var(--foreground)]">{formatNaira(customer.total_spent)}</p>
                    </div>
                    <div>
                      <p className="text-[var(--muted-foreground)]">Last Order</p>
                      <p className="font-medium text-[var(--foreground)]">{lastOrderLabel(customer)}</p>
                    </div>
                  </div>
      ...
      {selectedCustomer && (
        <div className="fixed inset-0 z-50 flex items-end sm:items-center justify-center">
          ...
          <div className="relative z-10 w-full sm:max-w-lg max-h-[85vh] overflow-y-auto rounded-t-2xl sm:rounded-2xl border border-[var(--border)] bg-[var(--card)] shadow-xl">
            ...
            {detailLoading ? (
              <div className="px-6 pb-6 rounded-b-2xl">
                <LoadingState />
              </div>
            ) : detailError ? (
              <div className="px-6 pb-6">
                <div className="flex flex-col items-center justify-center gap-3 rounded-xl border border-[var(--border)] bg-[var(--background)] py-8">
                  <p className="text-[13px] text-[var(--muted-foreground)]">
                    {detailError}
                  </p>
                  <button
                    onClick={() => void loadDetail(selectedCustomer.id)}
                    className="inline-flex items-center gap-1.5 rounded-lg bg-[var(--primary)] px-3 py-1.5 text-[12px] font-medium text-[var(--primary-foreground)] hover:opacity-90 transition-opacity"
                  >
                    <RefreshCw className="h-3.5 w-3.5" />
                    Try Again
                    ...
                    <div className="flex items-center gap-3">
                      ...
                      <div className="min-w-0">
                        ...
                        <p className="text-[13px] text-[var(--foreground)]">
                          {formatJoined(detail.created_at)}
                        </p>
                      </div>
                    </div>
                  </div>
                ...
                <div className="px-6 pb-4">
                  <div className="grid grid-cols-2 gap-3">
                    ...
                    <div className="rounded-xl border border-[var(--border)] bg-[var(--background)] p-3">
                      ...
                      <p className="mt-0.5 text-xl font-bold text-[var(--foreground)]">
                        {formatNaira(detail.total_spent)}
                      </p>
                    </div>
                  </div>
                </div>
                    ...
                    <div className="space-y-2">
                      ...
                      {detail.orders.length > 5 && (
                        <p className="text-center text-[12px] text-[var(--muted-foreground)] pt-1">
                          +{detail.orders.length - 5} more orders
                        </p>
                      )}
                    </div>
                  )}
